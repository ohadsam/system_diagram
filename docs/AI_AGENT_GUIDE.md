...
| Change 3D Presentation Mode (geometry, rendering, visual kinds, the "Realistic Room" mode, Camera Tour, or video/pptx export) | `js/core/scene3dLayout.js` (pure 2D→3D mapping + `getVisualKind` shape→silhouette mapping + cable direction/color, unit-testable) + `js/core/cameraTour.js` (pure tour math: `lerpAngle`/`easeInOutCubic`/`interpolateShot`/`computeAutoTourShots`, unit-testable) + `js/render3d/scene3dRenderer.js` (all Three.js/WebGL plus the tour's mutable playback *state* — the only importer of `vendor/three.module.min.js`; **must** call the returned `dispose()` on close, it holds a real GPU context) + `js/core/scene3dMode.js` (on/off pub-sub) + `js/canvas/scene3dOverlay.js` (overlay UI, incl. "🎯 Reset View", "🏢 Realistic Room", and the "🎬 Camera Tour" panel) + `js/io/export3dVideo.js` (drives playback via `animationPlayback.js#setFrozen(true)` + manual `nextStep()`, or `controller.playTourForExport()` if a tour is configured; records via `captureStream`/`MediaRecorder`) + `js/io/export3dPptx.js` (one slide per tour shot/animation step, via `controller.captureStillFrame()`). **Read** docs/ARCHITECTURE.md's "3D Presentation Mode" section before touching the orbit camera, the camera auto-fit/fog, `buildNodeVisual`'s per-`visualKind` geometry, the video-export step loop, or the Camera Tour state machine — in particular, don't map a new shape's `node.w`/`node.h` straight into a 3D box's width/depth without checking it's actually a spatial footprint (a lifeline's `h` isn't, see the gotcha there); if adding a new enclosing-geometry feature (room walls, etc.), remember the camera orbits at a fixed *spherical* distance regardless of angle, so any wall must be shaped with constant radius at every angle (a cylinder, not flat planes) with the wheel-zoom ceiling capped inside it, or it'll clip through from some rotation; a still-frame capture off this renderer (`captureStillFrame()`) must call `render()` and `toDataURL()` back-to-back with no `await` in between, since the renderer has no `preserveDrawingBuffer`; and don't judge this feature's visuals by reading the code alone — a whole batch of real bugs here (camera framing, lighting, label clipping) were only visible by actually rendering a diagram in it and taking a screenshot. |

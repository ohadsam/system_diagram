...
| Change Sticky Notes (quick-add button/context-menu item) or node rotation | `js/canvas/canvas.js#addStickyNote` (thin wrapper around `addCustomShapeNode`'s new third `extraOverrides` param — `iconVisible: false` + a randomized tilt) wired from `toolbar.js#buildQuickCreateGroup`, `canvas.js#openCanvasContextMenu`, and `commandPaletteModal.js`. The `rotation` field itself (`core/project.js`, degrees -180..180, default 0) is generic — any shape, set from `styleEditor.js`'s "Rotation" field, rendered via a `--node-rotation` CSS custom property (`canvas/node.js#updateNodeEl`, `css/node.css`) using the same fallback-composition trick as `--node-extra-shadow` below. |
...
| Change Group/Ungroup or mixed component+connector selection | `js/canvas/canvas.js` (`groupSelection`/`ungroupSelection`/`selectNode`/`beginMarquee`/`duplicateSelection`) + `js/toolbar/toolbar.js#renderContextRow` — `groupSelection` is also reachable from the right-click context menu (`groupMenuItem`), alongside "Group & Shrink" below |
| Change "Group & Shrink" (right-click a 2+ selection → collapse to one placeholder) | `js/canvas/canvas.js` (`groupAndShrinkSelection`/`expandShrunkGroup`/`dissolveShrunkGroup`, the `render()` hidden-member/edge-redirect logic, `renderGroupBackgrounds`'s anchor-rect override) + `js/canvas/shrinkGroups.js#computeShrunkGroups` (pure) + `core/project.js`'s `shrunkAnchorId` field. The placeholder's *face* is a small live composite of every member's own box/icon (`js/canvas/shrinkThumbnail.js#computeShrinkThumbnail` (pure) + `canvas/node.js#buildShrinkThumbnailBody`), not its own normal content — see docs/ARCHITECTURE.md's own "Group & Shrink" section, including **both** redesign notes, before touching this. |
| Change the suggested-flow-diagram "+ Add" miniature (details panel and the placement-time popover) | `js/canvas/canvas.js#attachSuggestedPatternAsMiniature` — creates the pattern via `instantiatePatternAtPoint(..., { silent: true })`, then shrinks/repositions it onto the host node's corner using the same `groupId`/`shrunkAnchorId` fields "Group & Shrink" above uses, plus a new `node.attachedHostId` (set on the anchor, naming the host) that `canvas.js#selectNode` follows in both directions so dragging/nudging the host carries the miniature (and its hidden group members) along — see docs/ARCHITECTURE.md's "Gotcha found via live QA" note right after the flow-diagram-suggestions section. Keep `instantiatePatternNearNode` (still used by drag-a-pattern-onto-a-node) separate — a deliberate drag-drop placement should stay full size. |
...
| Change pinch-to-zoom / two-finger rotate | `js/canvas/touchGeometry.js` (pure math: `touchPointDistance`/`touchPointAngleDeg`/`normalizeRotationDeg`, unit-testable) + `js/canvas/canvas.js` (`wireTouchGestures`, `beginTouchPinchRotate`) + `js/canvas/touchGestureCoordinator.js` (the shared cancel-hook `beginPan`/`nodeInteractions.js`'s drag gestures register into, so a second finger can tear one down first) — see docs/ARCHITECTURE.md's "8 UX/UI additions" section for why the pointer tracking lives at the `window` level, not `viewportEl` |
| Change Sketch Mode | `io/uiPrefs.js#sketchMode` + `index.html`'s `#sketch-wobble` SVG filter + `css/canvas.css`'s `body.sketch-mode` rules + the toggle button in `toolbar.js` |
| Change Presenter Mode's Spotlight or Remote Control | `js/toolbar/kioskModeUi.js` (`.kiosk-controls` flex row, spotlight pointer tracking) + `js/collab/presenterRemote.js` (PeerJS host/guest pairing, reuses `collab/peerjsCollab.js`) + `js/modals/presenterRemoteModal.js` (QR code via `vendor/qrcode.js`) + `remote.html` (the standalone phone-side page) |
| Change live-collaboration cursors | `js/collab/cursorSync.js` (`startCursorSync`, wired alongside `collab/collabSession.js#startCollabSession` in `modals/collaborationModal.js`) + `.collab-remote-cursor` in `css/canvas.css` |
| Change Inline Diagnostics (canvas lint badges) | `io/uiPrefs.js#inlineLintBadges` + `js/canvas/canvas.js#render` (now exported — see its own doc comment for why) + `js/canvas/node.js` (`.node-lint-badge`) + `css/node.css` |
| Change the Keyboard Shortcuts reference modal | `js/modals/keyboardShortcutsModal.js` — hand-maintained, no registry to introspect; add a row here whenever a new global shortcut is added to `main.js#initKeyboardShortcuts` |
...
| Change 3D Presentation Mode (geometry, rendering, visual kinds, the "Realistic Room" mode, Camera Tour, video/pptx export, or the control bar's dock position/compact mode) | `js/core/scene3dLayout.js` (pure 2D→3D mapping + `getVisualKind` shape→silhouette mapping + cable direction/color, unit-testable) + `js/core/cameraTour.js` (pure tour math: `lerpAngle`/`easeInOutCubic`/`interpolateShot`/`computeAutoTourShots`, unit-testable) + `js/render3d/scene3dRenderer.js` (all Three.js/WebGL plus the tour's mutable playback *state* — the only importer of `vendor/three.module.min.js`; **must** call the returned `dispose()` on close, it holds a real GPU context) + `js/core/scene3dMode.js` (on/off pub-sub) + `js/canvas/scene3dOverlay.js` (overlay UI, incl. "🎯 Reset View", "🏢 Realistic Room", the "🎬 Camera Tour" panel, and the "⚙️ Layout" panel driving `io/uiPrefs.js`'s `scene3dBarPosition`/`scene3dBarCompact`) + `js/io/export3dVideo.js` (drives playback via `animationPlayback.js#setFrozen(true)` + manual `nextStep()`, or `controller.playTourForExport()` if a tour is configured; records via `captureStream`/`MediaRecorder`) + `js/io/export3dPptx.js` (one slide per tour shot/animation step, via `controller.captureStillFrame()`). **Read** docs/ARCHITECTURE.md's "3D Presentation Mode" section before touching the orbit camera, the camera auto-fit/fog, `buildNodeVisual`'s per-`visualKind` geometry, the video-export step loop, the Camera Tour state machine, or the `.scene3d-bar[data-position=...]` dockable-bar CSS — in particular, don't map a new shape's `node.w`/`node.h` straight into a 3D box's width/depth without checking it's actually a spatial footprint (a lifeline's `h` isn't, see the gotcha there); if adding a new enclosing-geometry feature (room walls, etc.), remember the camera orbits at a fixed *spherical* distance regardless of angle, so any wall must be shaped with constant radius at every angle (a cylinder, not flat planes) with the wheel-zoom ceiling capped inside it, or it'll clip through from some rotation; a still-frame capture off this renderer (`captureStillFrame()`) must call `render()` and `toDataURL()` back-to-back with no `await` in between, since the renderer has no `preserveDrawingBuffer`; a new main-row button belongs in `makeSceneBtn`'s icon+label span pattern (not a flat `text:`) so compact mode and a real `title`/`aria-label` both keep working on it automatically; and don't judge this feature's visuals by reading the code alone — a whole batch of real bugs here (camera framing, lighting, label clipping) were only visible by actually rendering a diagram in it and taking a screenshot. |
...
| Change a step's entrance style (Fade/Slide up/Zoom in/Draw) or its auto-hide-after-time | `js/core/project.js#ANIMATION_ENTRANCE_STYLES`/`createAnimationStep`'s `entranceStyle`/`hideAfterMs` fields + `js/core/animationPlayback.js`'s `expiredStepIds`/`syncExpiryToRevealedCount`/`armExpiryFor`/`rearmExpiryForRevealed` (the hide-after-time state machine — separate from, but parallel to, the existing next-step auto-advance timer) + `js/canvas/canvas.js#applyAnimationVisibility`/`applyEdgeDrawEntrance` (rendering) + `js/canvas/canvas.js#setAllStepsEntranceStyle`/`setAllStepsHideAfterMs` (bulk) + `js/panel/animationPanel.js`'s per-step "Entrance"/"Hide after" controls and their own bulk row. **Read** docs/ARCHITECTURE.md's "Entrance styles and auto-hide-after-time" subsection before touching either — in particular why 'draw' only applies to a *solid* edge (an inline style always beats the same property set as a presentation attribute, so it would otherwise clobber a Dashed/Dotted connector's own pattern), and why the hide-after timer needs its own full `syncExpiryToRevealedCount` bookkeeping rather than reusing `revealedCount` directly. |
| Change "🗑️ Remove All"/"Remove Selected" or scope any Diagram Animation bulk action to checked steps only | `js/canvas/canvas.js#removeAnimationSteps` (bulk sibling of `removeAnimationStep`) + every bulk setter's trailing optional `stepIds` param (`toStepIdSet` helper — omitted/null means "every step," matching each function's original behavior) + `js/panel/animationPanel.js`'s `selectedStepIds` (module-level, ephemeral, checkbox per row in "In animation" + "Select all") and its `scopeIds`/`scopeLabel` computed once per render, feeding every bulk row's label/button text so which meaning ("all" vs. "N selected") a click will use is never ambiguous. **Read** docs/ARCHITECTURE.md's "Entrance styles and auto-hide-after-time" subsection ("Bulk actions can also be scoped...") before adding a new bulk action — it should take the same trailing `stepIds` param, not a separate one-off mechanism. |
| Change Diagram Animation's "+ Add All"/"+ Add Selected From Canvas" quick-add row (right under "Play Animation") | `js/panel/animationPanel.js#buildQuickAddRow` — "+ Add All" is `canvas.js#addAllToActiveAnimation`, just **moved** here from `buildAddMoreSection` (not duplicated — see docs/ARCHITECTURE.md's "Quick-add row" subsection for why a clone would break existing `hasText: '+ Add All'` e2e locators); "+ Add Selected From Canvas" is new, reading `store.getSelection()` directly and passing it to `canvas.js#addAnimationStep` as a group, same mechanism the canvas right-click "Add Selection to Animation" already used. Needs its own `store.subscribe('selection', render)` in `open()` (alongside the existing `'change'` subscription) to keep its live count in sync with canvas selection changes made while the panel is already open — `'selection'` and `'change'` are separate store events. |
...
| Change "Animation-only" content (hide a node from the normal canvas/exports, keep it for Diagram Animation) | `js/core/project.js#createNode`'s `animationOnly` field + `js/canvas/canvas.js#isAnimationOnlyVisible`/`applyAnimationOnlyVisibility` (a **second, independent** visibility pass from `render()`'s own `hiddenNodeIds`/`style.display` handling — called from both `render()` and the `onAnimationChange` listener, since starting/stopping playback never fires a store `'change'` event) + `css/node.css`'s `.anim-only-hidden` + `js/panel/detailsPanel.js`'s checkbox. **Read** docs/ARCHITECTURE.md's "Animation-only content, playback controls, and persisted presenter markup" section before touching this — the two-call-site requirement is easy to silently drop. |
| Change Diagram Animation's Restart/Pause-Play controls or the freeze+draw overlay's tools | `js/core/animationPlayback.js`'s `goToStart`/`setDrawingActive`/`isDrawingActive`/`drawingActive` (a strict *subset* of `frozen` — drawing always freezes, a plain pause does not draw) + `js/canvas/animationOverlay.js` (Restart/Pause-Play buttons, the Pen/Highlighter/Text tool row) + `js/main.js#initKeyboardShortcuts` (Space = pause/play, Home/`r` = restart, `d` = draw toggle, Escape now unwinds drawing → paused → stopped one level at a time). |
| Change persisted presenter annotations (drawings/text that reappear on replay, or get baked into PPTX/video export) | `js/core/project.js#createAnnotation`/`validateAnnotations` (canvas/world-space, **not** screen pixels — see `viewport.js#screenToCanvas`/`canvasToScreen`) + `js/canvas/canvas.js#saveAnimationStepAnnotations` (validates before persisting) + `js/canvas/animationOverlay.js`'s `sessionAnnotationsByStepId`/`redrawAnnotationLayer` (session-local override, since `animationPlayback.js`'s own `steps` snapshot is frozen at `startPlayback` time) + `js/io/exportImage.js#captureDiagramCanvas`'s `annotations` option (bakes them into an exported frame using the capture's own `bounds`/`scale`) + `js/io/exportAnimationPptx.js`/`js/io/exportAnimationVideo.js` (pass `step.annotations` through) + `js/io/exportAnimation.js` format v3 (`label`/`annotations`/an animation's own `notes`). **Read** docs/ARCHITECTURE.md's "Animation-only content, playback controls, and persisted presenter markup" section before changing the coordinate space — getting this wrong silently misplaces every mark the moment the viewport differs from when it was drawn. |
| Change Diagram Animation's duration indicator, presenter overview notes, Duplicate step/Animation, custom step labels, Randomize/Preview entrance, or drag-to-reorder | `js/core/animationVideoTiming.js#computeTotalDurationMs` (duration) + `js/core/project.js#createAnimation`'s `notes` (overview, distinct from a step's own `notes`) + `js/canvas/canvas.js#duplicateAnimationStep`/`#duplicateAnimation`/`#randomizeStepsEntranceStyle`/`#previewStepEntrance`/`#moveAnimationStepToIndex` + `js/panel/animationPanel.js` (all the corresponding buttons/fields, plus the step row's native HTML5 `draggable` wiring). |
...
- **A large blob download (roughly ~1MB+, e.g. an image-heavy `.pptx`) can
  ...
  (blob creation + the triggering `<a download>` click, via
  `utils/download.js#downloadBlob`) finishes client-side in well under a
  second.** Found while testing `io/export3dPptx.js`'s new "📊 Export 3D
  Presentation" button: a short-timeout `page.waitForEvent('download')`
  looked exactly like a hung/broken feature (no error, no console output,
  test just times out), and only a persistent `page.on('download', ...)`
  listener proved the download really does fire — just much later than a
  default `waitForEvent` or test timeout allows. A small blob (a JSON/text
  export) or a short recording (the sibling video-export test) doesn't show
  this delay, so it's specific to large-blob downloads, not download
  interception being broken in general. Not a real product slowness — a
  real browser doesn't have this issue — but any e2e test downloading a
...
- **A component/layout invariant computed relative to one element's box
  ...
  flag as relevant.
- **Starting/stopping Diagram Animation playback never fires the store's
  `'change'` event** (`core/animationPlayback.js` is deliberately its own
  pub-sub, separate from `core/store.js` — see its own header comment). Any
  canvas-visibility logic that depends on "is playback active right now"
  (e.g. `canvas.js#isAnimationOnlyVisible`) must therefore be re-run from
  the `onAnimationChange` listener `initCanvas` already registers, **not
  only** from the main `render(state)` that `store.subscribe('change', ...)`
  drives — an `animationOnly` node stayed permanently hidden through an
  entire Play/pause/Restart cycle the first time this was missed, only
  fixed by an e2e test that actually clicked Play and checked, since
  reading the code alone made the single-call-site version look complete.
- **A persisted, presenter-drawn annotation must be stored in canvas/world
  space (`viewport.js#screenToCanvas`), never raw screen pixels** — the
  entire point of persisting it is to redraw correctly across a different
  pan/zoom on replay, or inside `io/exportImage.js#captureDiagramCanvas`'s
  own independently-framed capture. If you add a new kind of annotation,
  give it the same treatment `core/project.js#createAnnotation` already
  uses and convert at the two boundaries only: `screenToCanvas` when
  recording a pointer event, `canvasToScreen` (or the export capture's own
  equivalent `bounds`/`scale` math) when drawing it back.
